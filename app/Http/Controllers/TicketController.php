...
class TicketController extends Controller
{
    ...
    public function report(){
        ...
        $tickets = DB::select("SELECT tickets.id, tickets.ticket_no, u.name AS user, departments.name AS dept, ticket_categories.name AS nature_of_problem, a.name AS assigned_to, tickets.subject, tickets.description, tickets.status, tickets.created_at, tickets.attachment, tickets.resolution FROM tickets INNER JOIN users AS u ON tickets.user_id = u.id INNER JOIN departments ON tickets.department = departments.id INNER JOIN users AS a ON tickets.assigned_to = a.id INNER JOIN ticket_categories ON tickets.nature_of_problem = ticket_categories.id WHERE tickets.created_at BETWEEN CONVERT(?, DATETIME) AND CONVERT(?, DATETIME) ORDER BY tickets.id DESC", [$newDateFrom, $newDateTo]);
    ...
    public function genReport(Request $request){
        ...
        if(isset($request->cbPending) && !isset($request->cbOngoing) && !isset($request->cbDone)){
            $status = " AND tickets.status = 'PENDING'";
            $cbp = 1;
            $ongoing = 0;
            $done = 0;
        }elseif(!isset($request->cbPending) && isset($request->cbOngoing) && !isset($request->cbDone)){
            $status = " AND tickets.status = 'ONGOING'";
            $cbo = 1;
            $pending = 0;
            $done = 0;
        }elseif(!isset($request->cbPending) && !isset($request->cbOngoing) && isset($request->cbDone)){
            $status = " AND tickets.status = 'DONE'";
            $cbd = 1;
            $pending = 0;
            $ongoing = 0;
        }elseif(isset($request->cbPending) && isset($request->cbOngoing) && !isset($request->cbDone)){
