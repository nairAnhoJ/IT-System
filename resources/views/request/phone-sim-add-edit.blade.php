<x-app-layout>
    ...
    <div style="height: calc(100vh - 65px);" class="py-10 px-80 text-gray-200 w-screen overflow-x-auto">
        ...
        <form action="{{ $Action == 'add' ? route('reqPhoneSim.store') : route('reqPhoneSim.update'); }}" method="POST">
            @csrf
            ...
            <label for="item" class="{{ $Action == 'edit' ? 'opacity-50 pointer-events-none' : ''; }} mt-5 block text-sm font-medium text-white">Select Item Type</label>
            <select {{ $Action == 'edit' ? 'disabled' : ''; }} required id="item" name="item" autocomplete="off" class="disabled:opacity-50 disabled:pointer-events-none border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white">
            ...
            <div class="mt-5">
                ...
                <a href="{{ route('reqPhoneSim.index') }}" class="inline-block text-center w-24 text-white focus:outline-none focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 bg-gray-800 hover:bg-gray-700 focus:ring-gray-700 border-gray-700">Back</a>
            </div>
        </form>
    </div>
